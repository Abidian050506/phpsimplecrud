...
if($_GET['aksi'] == 'inputmobil'){
...
} elseif($_GET['aksi'] == 'updatemobil'){
    // Mengambil data mobil dari form edit menggunakan metode POST dan menyimpannya dalam array
    $dataMobil = [
        'id' => $_POST['id'],
        'kode' => $_POST['kode'],
        'nama' => $_POST['nama'],
    ...
    if($update){
        ...
        header("Location: ../master-prodi-edit.php?id=".$dataMobil['id']."&status=failed");
    }
} elseif($_GET['aksi'] == 'deletemobil'){
    // Mengambil id prodi dari parameter GET
    $id = $_GET['id'];
    // Memanggil method deleteProdi untuk menghapus data prodi berdasarkan id
    $delete = $master->deleteMobil($id);
    if($delete){
